...
public class InFileSessionStore implements SessionStore {

	private final KeySignalAddressSerializer keySerializer = new KeySignalAddressSerializer();
	private final ConcurrentMap<SignalProtocolAddress, byte[]> sessions = (ConcurrentMap<SignalProtocolAddress, byte[]>) DbHolder
			.getDB().hashMap("sessions").keySerializer(keySerializer).createOrOpen();
